...
class QMUIMediaImagesProvider : QMUIMediaDataProvider {
    ...
    override suspend fun provide(
        ...
        return withContext(Dispatchers.IO) {
            ...
            val defaultPhotoBucket =
                MutableMediaPhotoBucket(QMUIMediaPhotoBucketAllId, QMUIMediaPhotoBucketAllName)
            buckets.add(defaultPhotoBucket)
            list.forEach { model ->
                defaultPhotoBucket.list.add(model)
                if (model.name.isNotBlank()) {
                    val bucket = buckets.find { it.id == model.bucketId }
...
fun Cursor.readLong(columnName: String): Long =
    getColumnIndexAndDoAction(columnName) { getLongOrNull(it) } ?: 0

fun Cursor.readString(columnName: String): String =
    getColumnIndexAndDoAction(columnName) { getStringOrNull(it) } ?: ""

fun Cursor.readInt(columnName: String): Int =
    getColumnIndexAndDoAction(columnName) { getIntOrNull(it) } ?: 0
