...
fun loadLongImage(
    ...
    val cacheStatistic =
        QMUIBitmapRegionCacheStatistic(cacheTimeoutForLazyLoad, cacheCountForLazyLoad)
    return loadLongImage(ins, preferredSize, options, fit) { regionDecoder ->
        val w = regionDecoder.width
        val h = regionDecoder.height
        val pageHeight = if (preferredSize.width > 0 && preferredSize.height > 0) {
            ...
            if (i < preloadCount) {
                ...
                ret.add(
                    QMUIBitmapRegionProvider(
                        bm.width,
                        bm.height,
                        QMUIAlreadyBitmapRegionLoader(bm)
                    )
                )
            } else {
                val finalTop = top
...
private class QMUIBitmapRegionCacheStatistic(
    ...
    private val cacheJobs =
        object : LruCache<QMUICacheBitmapRegionLoader, Job>(cacheCountForLazyLoad) {
            override fun entryRemoved(
                evicted: Boolean,
                key: QMUICacheBitmapRegionLoader?,
                oldValue: Job?,
                newValue: Job?
            ) {
